    ...
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-3 sm:px-6 sticky top-0 z-20 shadow-xs">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <span className="text-xs text-slate-500 font-medium hidden md:inline">
          Hệ thống Quản lý Dữ liệu Doanh nghiệp
        </span>
      </div>

      {/* Right side: Tech Map link, Notifications, User info, Logout */}
      <div className="flex items-center gap-2 sm:gap-3">
        {/* Nút chuyển nhanh đến Xem Tổng Quát (Tech Map) */}
        <Link
          to="/system-overview"
          className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-semibold bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white shadow-sm shadow-blue-500/20 transition-all cursor-pointer shrink-0"
        ...
        <div className="relative">
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100 relative transition-colors cursor-pointer"
            title="Thông báo"
          >
          ...
          {showNotifMenu && (
            <div className="absolute right-0 mt-2 w-72 sm:w-80 max-w-[calc(100vw-2rem)] bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <button
            onClick={logout}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 hover:text-rose-600 hover:bg-rose-50 transition-colors cursor-pointer"
            title="Đăng xuất"
          >

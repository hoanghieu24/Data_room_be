...
exports.create = async (req, res) => {
  try {
    ...
    if (existingPosition) {
      return res.status(400).json({ msg: "Chức danh đã tồn tại" });
    }

    const id = await Positions.createPosition(dto);
    return res.json({ msg: "Tạo chức danh thành công", position_id: id });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ msg: "Lỗi tạo chức danh" });
  }
};
...
exports.update = async (req, res) => {
  try {
    ...
    if (updated === 0) {
      return res
        .status(404)
        .json({ msg: "Chức danh không tồn tại hoặc đã bị vô hiệu" });
    }

    return res.json({ msg: "Cập nhật chức danh thành công" });
  } catch (err) {
    console.error("update position error:", err);
    return res.status(500).json({ msg: "Lỗi cập nhật chức danh" });
  }
};
...
exports.deleteById = async (req, res) => {
  try {
    ...
    return res.json({ msg: "Xóa chức danh thành công" });
  } catch (err) {
    console.error("delete position error:", err);
    return res.status(500).json({ msg: "Lỗi xóa chức danh" });
  }
};
...
exports.deleteByIds = async (req, res) => {
  try {
    ...
    if (deletedCount === 0) {
      return res.status(404).json({ msg: "Không có chức danh nào bị xóa" });
    }

    return res.json({ msg: `Xóa thành công ${deletedCount} chức danh` });
  } catch (err) {
